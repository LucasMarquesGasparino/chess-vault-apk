...
KEYSTORE="$PROJECT_DIR/chess-vault-release.keystore"
...
"$TOOLS_DIR/zipalign" -f -p 4 "$OUT/unsigned.apk" "$OUT/ChessVault-aligned.apk"

if [ ! -f "$KEYSTORE" ]; then
    echo ">> Gerando keystore..."
    keytool -genkeypair -noprompt \
        -keystore "$KEYSTORE" \
        -storepass chessvault \
        -keypass chessvault \
        -alias chessvault \
        -keyalg RSA -keysize 2048 -validity 10000 \
        -dname "CN=ChessVault, OU=Local, O=ChessVault, L=Local, ST=Local, C=BR"
fi

echo ">> Assinando..."
"$TOOLS_DIR/apksigner" sign \
    --ks "$KEYSTORE" \
    --ks-pass pass:chessvault \
    --key-pass pass:chessvault \
    --out "$OUT/ChessVault.apk" \
    "$OUT/ChessVault-aligned.apk"

echo ">> Verificando assinatura..."
"$TOOLS_DIR/apksigner" verify --verbose "$OUT/ChessVault.apk"

DOCUMENTS_DIR=/storage/emulated/0/Documents
DOWNLOADS_DIR=/storage/emulated/0/Download
mkdir -p "$DOCUMENTS_DIR" "$DOWNLOADS_DIR" 2>/dev/null || true
cp "$OUT/ChessVault.apk" "$DOCUMENTS_DIR/ChessVault.apk" 2>/dev/null || echo "Aviso: não foi possível copiar para Documents"
cp "$OUT/ChessVault.apk" "$DOWNLOADS_DIR/ChessVault.apk" 2>/dev/null || echo "Aviso: não foi possível copiar para Download"

echo ""
echo "✓ APK criado: $OUT/ChessVault.apk"
ls -lh "$OUT/ChessVault.apk"
echo "✓ Copiado para: $DOCUMENTS_DIR/ChessVault.apk"
echo "✓ Copiado para: $DOWNLOADS_DIR/ChessVault.apk"
